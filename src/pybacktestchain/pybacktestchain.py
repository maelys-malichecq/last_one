...
class Information:
    ...
    def compute_information(self, t : datetime):  
        ...
        # kurtosis on data set
        data['kurtosis'] = data.groupby(self.company_column)['return'].transform(lambda x: x.kurt())

        # expected return by company on information set
        information_set['expected_return'] = data.groupby(self.company_column)['return'].mean().to_numpy()
        # kurtosis on information set
        information_set['kurtosis'] = data.groupby(self.company_column)['kurtosis'].first().to_numpy()
        # covariance matrix
    ...
    def compute_portfolio_bis(self, t : datetime,  information_set : dict):
        mu = information_set['expected_return']
        Sigma = information_set['covariance_matrix']
        kurtosis = information_set['kurtosis']

        gamma = 1 # risk aversion parameter
        delta = 0.1  # kurtosis aversion parameter
        n = len(mu)
        
        # objective function
        obj = lambda x: x.dot(Sigma).dot(x) + gamma/2 * x.dot(Sigma).dot(x) + delta * np.sum(kurtosis * x)
        # constraints
        cons = ({'type': 'eq', 'fun': lambda x: np.sum(x) - 1})
        # bounds, only allow long positions
        bounds = [(0, 1)] * n
