...
@dataclass
class ConstraintsModification(Information):
    ...
    def compute_portfolio(self, t: datetime, information_set):
        ...
        #build the x th percentile for the variance, can be optimized by taking other values
        #the goal is to get a min vol in order to keep dynamism on the portfolio
        variance_threshold = np.percentile(variances, 70) 
        #objective function: max square of the return and min vol
        obj = lambda x: -(x**2).dot(mu) + gamma / 2 * np.sqrt(x.dot(Sigma).dot(x))

        #sum of the weight = 1 and min for the variance 
        cons = (
            {'type': 'eq', 'fun': lambda x: np.sum(x) - 1},
            {'type': 'ineq', 'fun': lambda x: np.sqrt(x.dot(Sigma).dot(x)) - variance_threshold}
        )
        bounds = [(0, None)] * n
        x0 = np.ones(n) / n
        res = minimize(obj, x0, constraints=cons, bounds=bounds)
